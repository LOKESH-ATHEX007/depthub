...
$student_query = "SELECT st1.currentYear, st1.class, st1.department FROM st1 WHERE regno = $regno";
...
// Fetch announcements for the sidebar with department filtering
$announcements_query = "
...
announcements.forEach(item => {
    item.addEventListener('click', () => {
        ...
        fetch('php/fetch/mark_announcement_as_read.php?A_id=' + A_id)
            .then(response => {
                if (!response.ok) {
        ...
        // Fetch and display the announcement details
        fetch('?A_id=' + A_id)
            .then(response => {
                if (!response.ok) {
                    throw new Error('HTTP error! status: ' + response.status);
