<?php
session_start();
require_once "../includes/dbconn.php"; // Your database connection file
...
// Database connection (already in dbconn.php)
$conn = $db; // Assuming your connection is stored in $db

// Fetch available currentYear & class combinations for the dropdown
if (isset($_GET['fetch_classes'])) {
    if (isset($_SESSION['T_id'])) {
        // Teacher - show only classes they teach
        $T_id = $_SESSION['T_id'];
        $query = "SELECT DISTINCT currentYear, class 
                  FROM class_teacher_allocation 
                  WHERE dept_id = ? AND T_id = ?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("ii", $dept_id, $T_id);
    } else {
        // Admin - show all classes in their department
        $query = "SELECT DISTINCT currentYear, class 
                  FROM class_teacher_allocation 
                  WHERE dept_id = ?";
        $stmt = $conn->prepare($query);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // For teachers, verify they are allocated to this class
    if (isset($_SESSION['T_id'])) {
        $T_id = $_SESSION['T_id'];
        ...
        // For admin, just verify the class exists in their department
        $checkQuery = "SELECT * FROM class_teacher_allocation 
    ...
    if ($checkResult->num_rows === 0) {
        echo "Invalid class selection or you don't have permission to upload for this class.";
        exit;
    }
    $stmt->close();

    // Check if timetable for this year/class/dept already exists
    ...
    if ($checkResult->num_rows > 0) {
        echo "A timetable for this class has already been uploaded in your department.";
        exit;
    }
    $stmt->close();
