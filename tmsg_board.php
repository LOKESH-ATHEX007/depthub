...
<ul class="student-list" id="student-list">
    <?php
    // Get teacher's department first
    $teacher_dept_query = "SELECT department FROM teachers WHERE T_id = ?";
    $teacher_dept_stmt = $conn->prepare($teacher_dept_query);
    $teacher_dept_stmt->bind_param("s", $T_id);
    $teacher_dept_stmt->execute();
    $teacher_dept_result = $teacher_dept_stmt->get_result();
    $teacher_dept = $teacher_dept_result->fetch_assoc()['department'];

    // Then query students with department filter
    ...
    while ($row = $result->fetch_assoc()) {
        echo "<li class='student-item' 
                ...
                onclick='selectStudent(\"" . $row["email"] . "\", \"" . addslashes($row["stName"]) . "\")'>
                <span class='student-name'>" . $row["stName"] . "</span>
