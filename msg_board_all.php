...
// First get teacher's department
$dept_stmt = $conn->prepare("SELECT department FROM teachers WHERE T_id = ?");
$dept_stmt->bind_param("s", $T_id);
$dept_stmt->execute();
$dept_result = $dept_stmt->get_result();
$teacher_dept = $dept_result->fetch_assoc()['department'];

// Fetch unique classes and years for sorting dropdown (filtered by department)
$classQuery = mysqli_query($conn, "SELECT DISTINCT class FROM st1 WHERE department = '$teacher_dept' ORDER BY class ASC");
$yearQuery = mysqli_query($conn, "SELECT DISTINCT currentYear FROM st1 WHERE department = '$teacher_dept' ORDER BY currentYear ASC");

// Get selected sorting values from dropdown (if any)
$selectedClass = isset($_GET['class']) ? $_GET['class'] : "";
$selectedYear = isset($_GET['currentYear']) ? $_GET['currentYear'] : "";

// SQL conditions for sorting
$filters = ["s.department = '$teacher_dept'"]; // Always filter by department
$params = [];
...
if (!empty($selectedYear)) {
    $filters[] = "s.currentYear = ?";
    $params[] = $selectedYear;
}
$filterSQL = !empty($filters) ? "WHERE " . implode(" AND ", $filters) : "";

// Fetch students with sorting order
$query = "SELECT s.regno, s.email, s.stName, s.class, s.currentYear,
                 MAX(m.timestamp) AS last_message_time,
                 (SELECT COUNT(*) FROM messages 
                  WHERE sender_email = s.email AND receiver_email = ? AND is_read = 0) AS unread_count
          FROM st1 s
          LEFT JOIN messages m ON 
              (s.email = m.sender_email AND m.receiver_email = ?) 
              OR (s.email = m.receiver_email AND m.sender_email = ?)
          $filterSQL
          GROUP BY s.regno, s.email, s.stName, s.class, s.currentYear
          ORDER BY unread_count DESC, last_message_time DESC, s.currentYear ASC, s.class ASC, regno ASC";
...
$baseParams = [$_SESSION["user_email"], $_SESSION["user_email"], $_SESSION["user_email"]];
...
// Display "No students found" if empty result
...
<head>
    ...
    <link rel="stylesheet" href="styles/msg_all.css">
</head>
<body>
    <div class="container">
        <div class="sidebar">
            ...
            <div class="search-container">
                <input class="input" placeholder="Reg no, Name, Email" id="search" type="text" onkeyup="searchStudents()">
            ...
            <form method="GET">
                ...
                <select name="currentYear" onchange="this.form.submit()">
                    <option value="">Select Year</option>
                    <?php while ($row = mysqli_fetch_assoc($yearQuery)) { ?>
                        <option value="<?php echo $row['currentYear']; ?>" <?php if ($selectedYear == $row['currentYear']) echo "selected"; ?>>
                            <?php echo $row['currentYear']; ?>
                        </option>
                    <?php } ?>
                </select>
                ...
                <select name="class" onchange="this.form.submit()">
                    ...
                    <?php while ($row = mysqli_fetch_assoc($classQuery)) { ?>
                        <option value="<?php echo $row['class']; ?>" <?php if ($selectedClass == $row['class']) echo "selected"; ?>>
                            <?php echo $row['class']; ?>
                        </option>
                    <?php } ?>
                </select>
    ...
    <?php if ($result->num_rows > 0) { ?>
        <?php while ($row = $result->fetch_assoc()) { ?>
            <li class='student-item' 
                data-regno='<?php echo htmlspecialchars($row["regno"]); ?>' 
                data-email='<?php echo htmlspecialchars($row["email"]); ?>' 
                onclick='selectStudent("<?php echo htmlspecialchars($row["email"]); ?>", "<?php echo htmlspecialchars(addslashes($row["stName"])); ?>")'>
                
                <span class='student-name'>
                    <?php echo htmlspecialchars($row["stName"]); ?> 
                    (<?php echo htmlspecialchars($row["currentYear"] . "-" . $row["class"]); ?>)
                    <?php if (isset($row['department'])): ?>
                        <span class='student-dept'><?php echo htmlspecialchars($row['department']); ?></span>
                    <?php endif; ?>
                </span>
                
                <span class='unread-badge' id='unread-<?php echo htmlspecialchars($row["email"]); ?>'>
                    <?php echo ($row["unread_count"] > 0) ? htmlspecialchars($row["unread_count"]) : ""; ?>
                </span>
            </li>
        <?php } ?>
    <?php } else { ?>
        <li class="no-students-message">
            <img src="pic/no-students.svg" alt="No students found" class="no-students-icon">
            <div class="no-students-text">
                <p>No students found in your department</p>
                <p class="subtext">
                    <?php if (!empty($selectedClass) || !empty($selectedYear)): ?>
